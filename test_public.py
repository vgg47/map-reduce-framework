...
from .lib.graph import Graph, Node, NodeFromFile, NodeFromIter
from .lib import operations
from .lib import external_sort as exts


MiB = 1024 ** 2


########## LIGHT TESTS ON CORRECTNESS ##########


def test_word_count() -> None:
    graph = graphs.word_count_graph('docs')
...
def test_word_count_multiple_call() -> None:
    graph = graphs.word_count_graph('text', text_column='text', count_column='count')
...
def test_yandex_maps() -> None:
    ...
    assert expected == sorted(result, key=itemgetter('weekday', 'hour'))
...
def run_and_track_memory(callback: tp.Callable[[], T], limit: int) -> T:
    thread = memory_watchdog.MemoryWatchdog(limit)
    thread.start()
    result = callback()
    thread.stop()
    thread.join()
    maximum_memory_usage = thread.maximum_memory_usage
    assert maximum_memory_usage <= limit
    return result


def test_yandex_maps_heavy() -> None:
    graph = graphs.yandex_maps_graph(
        'travel_time', 'edge_length',
        enter_time_column='enter_time', leave_time_column='leave_time', edge_id_column='edge_id',
        start_coord_column='start', end_coord_column='end',
    ...
    result = run_and_track_memory(
        lambda: graph.run(travel_time=lambda: islice(cycle(iter(times)), len(times) * 10000),
...
######### GRAPHS STRUCTURE TESTS ##########

def test_structure_1() -> None:
    graph1 = Graph.graph_from_iter('rows').sort(['doc_id'])

    assert isinstance(graph1.tail, Node)
    assert isinstance(graph1.tail.operation, exts.ExternalSort)
    assert len(graph1.tail.parents) == 1
    assert isinstance(graph1.tail.parents[0], NodeFromIter)


def test_structure_2() -> None:
    graph1 = Graph.graph_from_iter('rows').sort(['doc_id'])

    graph2 = Graph.graph_from_iter('data') \
        .sort(['doc_id']) \
        .join(operations.InnerJoiner(), graph1, ['doc_id'])

    assert isinstance(graph2.tail.operation, operations.Join) # type: ignore
    assert len(graph2.tail.parents) == 2 # type: ignore
    assert isinstance(graph2.tail.parents[0].operation, exts.ExternalSort) # type: ignore
    assert graph2.tail.parents[1] == graph1.tail # type: ignore
